...
% Articles are published under the CC BY license, which allows for
% redistribution while ensuring authors receive proper credit.
% See https://creativecommons.org/licenses/ for details.
% Copyright and license information is automatically printed
% when using the journal style file.
...
\node[anchor=north west,xshift=0.5cm,yshift=-0.5cm]{\includegraphics[width=1in]{CCAlogo.png}};
\end{tikzpicture}
...
properties; combinatorial topology and $f$-vectors; Gröbner/initial
...
Authors publishing in CCA retain copyright of their work.  CCA
articles are published under the \href{\CCBYURL}{Creative Commons BY
  (Attribution) license}, which allows for redistribution while giving
...
be produced using pdfLaTeX. CCA uses the style file \verb|cca.sty|
which can be downloaded from
\href{https://github.com/Combinatorial-Commutative-Algebra/CCA_Journal}{the
  journal's repository}. Place this file in the same directory as your
article's TeX file to compile. The CCA logo and article identifiers
...
\item[Authors:] For each author, please enter their contact
  ...
  printed at the end.  Footnotes on author names are prohibited, with
  two exceptions (see Note \ref{footnotes}).
...
\item All sections should be automatically numbered. Do not include a
  table of contents.
\item The default fonts and font sizes are set by the \verb|cca.sty|
...
\item The \verb|cca.sty| file defines the following environments:
  ...
  sections.
\item Equations environments are also numbered within sections. CCA
  recommends numbering only those equations that are explicitly
  referred to in the rest of the article.
...
bibliographic references. All references appearing in the bibliography
must be cited in the text. Please make citations as explicit as
possible by including an environment number (e.g.: Theorem 2.1(a),
Example IV), a (sub)section number, or a page number.  Upon
compilation, backlinks from the references to the text will be
automatically inserted.

CCA recommends compiling a bibliography with BibTeX. CCA applies the
custom style \texttt{abbrvcca.bst}, which automatically produces arXiv
and DOI links for correctly formatted references. If using BibTeX,
please upload your \texttt{.bib} file along with your sources.
\begin{itemize}
\item Bibliographic information for published works can be freely
  retrieved from databases such as \href{https://zbmath.org/}{zbMath}
  and \href{https://mathscinet.ams.org/mrlookup}{MR Lookup}. Whenever
  possible, please include a Digital Object Identifier in the format
  \verb|doi = \{10.1000/182\}|. If a DOI is not available, a
  complete URL to the published work may be included using the
  \texttt{url} field.
\item References for \href{https://arxiv.org/}{arXiv} preprints can be
  obtained directly from the preprint abstract page using the ``Export
  BibTeX Citation'' link, or at \href{https://zbmath.org/}{zbMath}.
\end{itemize}
If not using BibTeX, please include your references in
\texttt{thebibliography} environment with a \verb|\bibitem| for each
entry. Please format your references as closely as possible to the
style in this sample file.  Whenever possible, please include arXiv
and DOI links after each reference, using the \verb|\arxiv| and
\verb|\doi| commands as shown in the source of this sample file.
...
\cite{eisenbud1994binomialideals,zbMATH00922152,zbMATH03545783,zbMATH00833410,zbMATH06413064}.
...
%BIBLIOGRAPHY
% CCA uses its own BibTeX style contained in the abbrvcca.bst file
% To format references using BibTeX, uncomment the lines below and
% replace refs with your .bib file.
% Upload your .bib file along with your TeX sources.

% \bibliographystyle{abbrvcca}
% \bibliography{refs}

% If not using BibTeX, use thebibliography environment with entries
% formatted as shown below (first and middle initials, etc.)
% CCA also recommends using \doi{...} and \arxiv{...} as shown below,
% to produce clickable links to bibliographic sources

\begin{thebibliography}{1}
\bibitem{eisenbud1994binomialideals}
D.~Eisenbud and B.~Sturmfels.
\newblock Binomial ideals, 1994.
\newblock \arxiv{alg-geom/9401001}

\bibitem{zbMATH00922152}
D.~Eisenbud and B.~Sturmfels.
\newblock Binomial ideals.
\newblock {\em Duke Math. J.}, 84(1):1--45, 1996.
\newblock \doi{10.1215/S0012-7094-96-08401-X}

\bibitem{zbMATH03545783}
M.~Hochster.
\newblock Cohen-macaulay rings, combinatorics, and simplicial complexes.
\newblock Ring {Theory} {II}, {Proc}. 2nd {Okla}. {Conf}. 1975, 171-223
  (1977)., 1977.

\bibitem{zbMATH00833410}
R.~P. Stanley.
\newblock {\em Combinatorics and commutative algebra.}, volume~41 of {\em Prog.
  Math.}
\newblock Basel: Birkh{\"a}user, 2nd ed. edition, 1996.
\newblock \doi{10.1007/b139094}

\bibitem{zbMATH06413064}
R.~H. Villarreal.
\newblock {\em Monomial algebras}.
\newblock Monogr. Res. Notes Math. Boca Raton, FL: CRC Press, 2nd ed. edition,
  2015.
\newblock \doi{10.1201/b18224}
...
\item A new section appended to the original paper by placing
  ...
  \end{center}
  before the end of the document in the accepted manuscript’s source
  file (using cca style). The date is the day author(s) submit the
