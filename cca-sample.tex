...
% CCA papers *must* begin with the following two lines. 
...
% PACKAGES
% Packages geometry, amssymb, amsthm, hyperref and microtype are already loaded. 
...
\usepackage{tikz}
\usepackage{enumitem}
\setlist{topsep=1pt,partopsep=1pt,parsep=1pt,itemsep=1pt}
...
CCA articles \textbf{must} begin with the following two lines
\begin{verbatim}
\documentclass[12pt]{article}
\usepackage[amsmath]{cca}
\end{verbatim}
The option \verb|amsmath| can be omitted if the \texttt{amsmath}
package is not needed.  The packages \texttt{geometry},
\texttt{amsthm}, \texttt{amssymb}, \texttt{hyperref}, and
\texttt{microtype} are automatically loaded. Please do not reload or
set any options for these packages. The packages \texttt{fullpage},
\texttt{authblk}, \texttt{babel} are explicitly forbidden, as is any
command that would change page or margin sizes.
...
\item The \verb|cca.sty| file defines the following environments:
  ...
  remark.  The environments are numbered sequentially within sections.
